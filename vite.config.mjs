...
function levelSavePlugin() {
  return {
    ...
    configureServer(server) {
      server.middlewares.use("/api/save-shepherd-map", (req, res) => {
        if (req.method !== "POST") {
          res.statusCode = 405;
          res.end();
          return;
        ...
        req.on("end", () => {
          try {
            ...
            if (!Array.isArray(data.trees) || !Array.isArray(data.spawns)) {
              res.statusCode = 400;
              res.end(JSON.stringify({ error: "bad payload" }));
              return;
            }
            const file = path.join(__dirname, "src/scenes/shepherd-map.json");
