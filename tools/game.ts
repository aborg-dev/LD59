...
async function ensurePage(url: string, width = 480, height = 720): Promise<Page> {
...
export async function launch(url: string, width = 480, height = 720): Promise<void> {
...
export async function getCircle(): Promise<CircleState> {
  return page!.evaluate(() => {
    ...
    const c = s.children.list.find((o: any) => o.type === 'Arc');
    return { x: c.x, y: c.y, radius: c.radius, gameWidth: s.scale.width, gameHeight: s.scale.height };
  });
}
...
export async function drag(fromX: number, fromY: number, toX: number, toY: number, steps = 5): Promise<void> {
  await page!.evaluate(({ fromX, fromY, toX, toY, steps }) => {
    const canvas = document.querySelector('canvas')!;
    const dispatch = (type: string, x: number, y: number) => {
      canvas.dispatchEvent(new MouseEvent(type, {
        clientX: x, clientY: y, bubbles: true, button: 0, buttons: type === 'mouseup' ? 0 : 1,
      }));
    };
    dispatch('mousedown', fromX, fromY);
    for (let i = 1; i <= steps; i++) {
      const t = i / steps;
      dispatch('mousemove', fromX + (toX - fromX) * t, fromY + (toY - fromY) * t);
    }
...
export async function isGameRunning(): Promise<boolean> {
  return page!.evaluate(() => (window as any).game?.constructor?.name === 'Game');
}
...
export async function eval_(expression: string): Promise<any> {
